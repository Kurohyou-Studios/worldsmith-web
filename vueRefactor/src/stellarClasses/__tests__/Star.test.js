...
import { colorTemperature2rgb as cTemp } from '@/colorTemp';
...
describe('Star class',()=>{
  ...
  describe('Instance operations',()=>{
    ...
    const testAge = 4.5;
    const testMass = 1;
    const testStar = new Star({name:testName,mass:testMass,age:testAge});
    ...
    test('A newly created star should be initialized with a name, creation date, and all the calculated properties of the star.',()=>{
      ...
      expect(testStar.serial).toEqual(['type','name','id','creation','children','mass','age']);
    ...
    describe('Serialization',()=>{
      ...
      test('toJSON() should return a JSON string version of the entire system',()=>{
        expect(JSON.stringify(testStar)).toEqual(JSON.stringify(expected));
      });
    });
  });
})